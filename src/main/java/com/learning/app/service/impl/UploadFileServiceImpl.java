...
public class UploadFileServiceImpl implements UploadFileService {
	...
	public String uploadMultipleFiles(MultipartFile[] files, String orderId)
			throws Exception {
		createWorkBook(files, orderId);
		return "success";
	}

	private void createWorkBook(MultipartFile[] files, String orderId)
			throws Exception {
		...
		cellStyleBatore.setAlignment(cellStyleBatore.ALIGN_CENTER);
		...
		// Parchuniye!!!!
		Font fontParchuniye = wb.createFont();
		sheet.addMergedRegion(new CellRangeAddress(5, 6, 0, 7));
		CellStyle cellStyleParchuniye = wb.createCellStyle();
		// font.setBoldweight(Font.BOLDWEIGHT_BOLD);
		fontParchuniye.setItalic(true);
		fontParchuniye.setColor(IndexedColors.BLACK.index);
		fontParchuniye.setFontHeightInPoints((short) 20);
		cellStyleParchuniye.setFont(fontParchuniye);
		cellStyleParchuniye.setAlignment(cellStyleParchuniye.ALIGN_RIGHT);

		XSSFRow row0 = sheet.createRow(5);
		XSSFCell cell0 = row0.createCell(0);
		cell0.setCellValue("fullfilled by Parchuniye");
		...
		for (MultipartFile file : files) {
			...
			XSSFClientAnchor anchor = drawing.createAnchor(0, 0, 500, 400, 0,
					x, 0, 0);
	 ...
	 * 
	 * CellStyle cellStyle = wb.createCellStyle(); CreationHelper createHelper =
	 * wb.getCreationHelper();
	 * cellStyle.setDataFormat(createHelper.createDataFormat
	 * ().getFormat("MMMM dd, yyyy"));
	 * 
	 * XSSFRow row0 = sheet.createRow(0); XSSFCell cell0 = row0.createCell(0);
	 * cell0.setCellValue("Parchuniye");
	 * 
	 * XSSFRow row1 = sheet.createRow(1); XSSFCell cell1 = row1.createCell(0);
	 * cell1.setCellValue("Kiryana Store");
	 * 
	 * XSSFRow row2 = sheet.createRow(2); XSSFCell cell2 = row2.createCell(0);
	 * cell2.setCellValue(new Date()); cell2.setCellStyle(cellStyle);
	 * 
	 * XSSFRow row3 = sheet.createRow(3); XSSFCell cell3 = row3.createCell(0);
	 * cell3.setCellValue(" ");
	 * 
	 * 
	 ...
	 * 
	 * // write this workbook to an Outputstream. wb.write(out); out.flush();
	 * out.close(); }
	 */

	public static XSSFPicture createPicture(XSSFClientAnchor anchor,
			int pictureIndex, XSSFDrawing drawing) throws Exception {
		Method m = XSSFDrawing.class.getDeclaredMethod("addPictureReference",
				int.class);
		m.setAccessible(true);
		PackageRelationship rel = (PackageRelationship) m.invoke(drawing,
		...
		m = XSSFPicture.class.getDeclaredMethod("setPictureReference",
	...
	public static CTOneCellAnchor createOneCellAnchor(XSSFDrawing drawing,
			XSSFClientAnchor anchor) {
		final int pixel2emu = 12700;
		CTOneCellAnchor ctAnchor = drawing.getCTDrawing().addNewOneCellAnchor();

		long cx = (anchor.getTo().getRowOff() - anchor.getFrom().getRowOff())
				* pixel2emu;
		long cy = (anchor.getTo().getColOff() - anchor.getFrom().getColOff())
				* pixel2emu;
		...
		try {
			Method mt = XSSFClientAnchor.class.getDeclaredMethod("setFrom",
					CTMarker.class);
